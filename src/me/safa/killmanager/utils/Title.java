...
public class Title {
	...
	public void sendTitle(Player player, Integer fadeIn, Integer fadeOut, Integer stay, List<String> titles,
			int duration) {
		...
		new BukkitRunnable() {
			...
			public void run() {
				...
				String[] split = titles.get(indexNumber++).split("%newline%");
	...
	public Object createTimesPacket(Player player, Integer fadeIn, Integer fadeOut, Integer stay, String title,
			String subtitle) {
		if (title != null)
			title = msgHelper.colorFormat(title);
		if (subtitle != null)
			subtitle = msgHelper.colorFormat(subtitle);

		try {
			// Times packet
			Object object = getNMSClass("PacketPlayOutTitle").getDeclaredClasses()[0].getField("TIMES")
					.get((Object) null);
			Object titleObject = getNMSClass("IChatBaseComponent").getDeclaredClasses()[0]
			...
			Object titlePacket = subtitleConstructor
					.newInstance(new Object[] { object, titleObject, fadeIn, stay, fadeOut });
			return titlePacket;
	...
	public Object createTitlePacket(Player player, Integer fadeIn, Integer fadeOut, Integer stay, String title,
			String subtitle, boolean isSubtitle) {
		if (title != null)
			title = msgHelper.colorFormat(title);
		if (subtitle != null)
			subtitle = msgHelper.colorFormat(subtitle);
		...
		try {
			...
			// Times packet
			Object object = getNMSClass("PacketPlayOutTitle").getDeclaredClasses()[0]
					.getField(isSubtitle ? "SUBTITLE" : "TITLE").get((Object) null);
			Object titleObject = getNMSClass("IChatBaseComponent").getDeclaredClasses()[0]
					.getMethod("a", new Class[] { String.class })
					.invoke((Object) null, new Object[] { "{\"text\":\"" + finalString + "\"}" });
			@SuppressWarnings("rawtypes")
			Constructor subtitleConstructor = getNMSClass("PacketPlayOutTitle")
					.getConstructor(new Class[] { getNMSClass("PacketPlayOutTitle").getDeclaredClasses()[0],
							getNMSClass("IChatBaseComponent"), Integer.TYPE, Integer.TYPE, Integer.TYPE });
